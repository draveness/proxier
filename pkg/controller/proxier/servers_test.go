...
func TestGroupServers(t *testing.T) {
	instance := framework.MakeBasicProxier("default", "group-server", []string{"v1", "v2"}, []int32{10, 20})
	...
	servicesToAdd, servicesToDelete := groupServers(instance, existingServices)

	assert.Equal(t, []*corev1.Service{}, servicesToDelete)
